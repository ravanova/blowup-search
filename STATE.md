...
| `K3` | 439 | FAN-OUT ×5, CONSTRUCTION — the composition floor | wave 4 redux on ONE rule: *evidence is a two-route agreement on a quantity the adversary cannot choose* (what leg 432 did and wave 4's gates did not). A gate that cannot be put in two-route form is DROPPED, not weakened. Scales checked against the paper's own asymptotics BEFORE the prereg is pushed (§71). Slot 5 the adversary, blind: a signal it can fake is NOT EVIDENCE whatever the worker's gate said | **IN FLIGHT** (×5; Q6 DROPPED, §75) |
