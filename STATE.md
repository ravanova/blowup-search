...
cache) — one run, one container, `UNVERIFIED`; `K1` owes the completed build and a second run. **All of that is now discharged, and as of 2026-09-11 the check no longer depends on the published olean cache at all: `K1` phase C rebuilt mathlib from source and got the same axioms byte for byte.**
...
| `K1` | 437 | SERIAL, **no time cap** | (a) does `lake build` complete; (b) `#print axioms` verbatim for `navier_stokes_breakdown_R3` and `_periodic`; (c) `sorryAx` reachable from either — YES/NO; (d) wall time and machine. GREEN = the standard three only → STOP AND REPORT. RED = `sorryAx` or any other axiom → STOP, ESCALATE, NO PUBLICATION, blind reproduction first. Prereg `leg_437_prereg.md`; **GREEN in 3 environments** (A; B fresh container `total_s` 5605; B laptop `total_s` 4272), same pin/mathlib/Comparator revs — `leg_437.md` §1–§2 | **GREEN, VERIFIED** — `K2` slot 5 blind |
| `K2` | 438 | FAN-OUT ×5 | (1) the 5 `PARTIAL` statements — what is missing, per statement; (2) the 4 `sorry` — each a challenge placeholder, none reachable from a main declaration, cross-checked against `K1`'s axiom output, which WINS on disagreement; (3) the Comparator's 4 `NOT-ESTABLISHED` checks — run them; (4) DeepMind byte-identity re-verified at upstream source, independently; (5) ADVERSARIAL VERIFIER, blind, a sample of 1–4. One file per agent, own branch, cherry-picked unedited. Prereg `leg_438_prereg.md` (run 1's four files pre-date it, §74) | **GREEN** (run 2 ×5, `leg_438.md`) |
| `K3` | 439 | FAN-OUT ×5, CONSTRUCTION | wave 4 redux on ONE rule: *evidence is a two-route agreement on a quantity the adversary cannot choose*; a gate that cannot be put in that form is DROPPED, not weakened. Prereg `leg_439_prereg.md` + amendment (§75) | **DONE — 0 of 5 live gates survive as EVIDENCE.** The blind adversary faked ALL 5. Q1 `YES`→NOT EVIDENCE; Q4 4 cells `NO`, 3 `YES` faked; Q5 `YES`→NOT EVIDENCE, its two routes were ONE closed form; Q2 UNDER-RESOURCED; Q3 NOT-INSTANTIATED→DROPPED; Q6 DROPPED pre-run. 3 prereg defects, all the Conductor's: §79 |
| `K4` | 440 | SERIAL | `writeup/7_confirmation/` quartet + one outsider document, verdict FIRST. Prereg `leg_440_prereg.md`, pushed with the IN FLIGHT row BEFORE any document | **LANDED — verdict published.** Kernel check GREEN and **VERIFIED** (one pin, 3 environments, 2 independent kernels; statements are Fefferman (C)/(D), **strictly weaker** than Thm 1.1). Olean INTEGRITY established; SEMANTIC MATCH **CLOSED 2026-09-11** (phase C, §81). `K3` zero. No position on priority. fig115; `confirmation_evidence.py` PASS |
