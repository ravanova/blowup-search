...
QUOTES = [
    ...
    },
    {
        ...
        "supports": ("the ledger's CLAIM LINE ('the isolated eigenvalues are only the "
                     "symmetry modes') exactly -- and NOT the ledger's note, see DRIFT below"),
    },
    {
...
# The one row-level DRIFT this audit found, stated as data so the test can gate on it.
DRIFT = [
    {
        ...
        "blast_radius": [
            {"site": "solver/literature_gates.py:50", "carries_drift": True,
             "text": "docstring: 'Theorem 2: the full point spectrum ... is exactly {0, 1}'"},
            {"site": "solver/literature_gates.py:487", "carries_drift": True,
             "text": "CLAIM_LEDGER row source + note"},
    ...
    },
    {
        ...
        "blast_radius": [
            {"site": "solver/literature_gates.py:600", "carries_drift": True,
             "text": "CLAIM_LEDGER row source parenthetical"},
            {"site": "LITERATURE_CHECK.md (verdict table row)", "carries_drift": True,
             "text": "'2302.12877 - fetched, NOT read' -- same stale parenthetical"},
            {"site": "capabilities.py:179", "carries_drift": True,
             "text": "'UNSEARCHED at primary source' -- leg 65's own finding, also unapplied"},
        ],
    },
]
...
def main():
    ...
    out = {
        ...
        "L6_drift": {
            "n_drift_rows": len(DRIFT),
            "n_verdicts_changed": sum(1 for d in DRIFT if d["verdict_changes"]),
            "rows": DRIFT,
        },
        "L7_gate": {
    ...
    print(f"L6  drift                  {len(DRIFT)} rows, "
          f"{out['L6_drift']['n_verdicts_changed']} verdicts changed")
    for d in DRIFT:
        print(f"      [{d['kind']}] {d['row_claim'][:58]!r}")
        print(f"          stored source: {d['stored_source']}")
