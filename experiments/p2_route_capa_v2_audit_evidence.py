"""fig80 -- Route-CAPA v2 (leg 292): rebuild the audit figure from the curated JSON.
...
`writeup/figures/fig80_route_capa_v2_audit.png` -- and never re-runs the sweep.
...
def main() -> int:
    if not DATA.exists():
        print(f"  SKIP fig80: {DATA} not present -- run the audit first")
        return 0
