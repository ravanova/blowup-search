...
CANDIDATES = [
    ...
    # ---------------- survives both screens ----------------
    {
        ...
        "object": (
            ...
            "gamma = 7/5, radially symmetric imploding self-similar profile at r = r^(3)"
        ),
        "ansatz": (
            "smooth self-similar IMPLODING profile (U^E, S^E) solving the compressible "
            "self-similar ODE system, at the non-integer similarity exponent r^(3) in "
            "(r_3(gamma), r_4(gamma)) = (1.070374, 1.094975) -- i.e. n = 3, the SMALLEST "
            "odd branch, lying entirely BELOW the borderline 7/6. Existence is "
            "UNCONDITIONAL: BCG Thm 1.1 (l.214) gives r^(3)(gamma) for every gamma > 1, "
            "with no 'large enough' attached, hence at gamma = 7/5. There delta_dis is in "
            "(-0.5778, -0.4302) < 0, so r^(3) lies OUTSIDE BCG's dominance window "
            "(7/6, r*) = (1.1666667, 1.1909830) -- the side obligation 4 requires, and the "
            "side BCG's own NS theorem (Thm 1.3, which needs delta_dis > 0) does not reach. "
            "[ANSATZ CORRECTED 2026-08-07 by leg 275 -- see ANSATZ_CORRECTION_2026_08_07 "
            "in CERTIFICATE_OBLIGATION.]"
        ),
...
CERTIFICATE_OBLIGATION = {
    ...
    "certificate_must_show": [
        "(1) [RE-POSED 2026-08-07 by leg 266, source writeup/novelty/verify_251.md sec C; "
        "see CORRECTION_2026_08_07 below] A RIGOROUS ENCLOSURE, in interval arithmetic, of "
        "the STABILITY STEP WITH THE DISSIPATIVE FORCING F_dis RETAINED, at a similarity "
        "exponent r OUTSIDE BCG's dominance regime -- i.e. the enclosed object itself "
        "carries the viscous term, rather than the viscous term being estimated against an "
        "enclosed inviscid object. Concretely, from BCG's own text: dissipation enters the "
        "DYNAMICALLY RESCALED (W,Z) system as a non-autonomous forcing F_dis (defined "
        "l.2130, on the RHS at l.2138-2139, named 'the dissipative forcing' at l.2142) "
        "carrying the explicit prefactor e^{(2 - r + (1-r)/alpha)s}; BCG's (eq:delta:dis) "
        "at l.489-491 sets -delta_dis = 2 - r + (1-r)/alpha and REQUIRES delta_dis > 0, "
        "equivalently (eq:r:restriction) r > 2*gamma/(gamma+1), precisely so that the "
        "forcing decays in self-similar time and 'can be treated as an error'. With their "
        "standing 1 < r < r* from (eq:rstar), the dominance window at gamma = 7/5 is "
        "r in (7/6, r*) = (1.1666667, 1.1909830), width 0.0243163. The certificate's "
        "r-coverage must lie outside it, at r <= 7/6, where F_dis no longer decays; the "
        "interval (1, 7/6] is non-empty, so this is statable as a certificate target. "
        "That is exactly leg 174's Grade A definition, and it is the "
    ...
    "CORRECTION_2026_08_07": (
        "OBLIGATION 1 RE-POSED by leg 266 (Route-P0TC). SOURCE OF THE CORRECTION: "
        "writeup/novelty/verify_251.md sec C, the independent verification of leg 251. "
        "WHAT WAS WRONG: obligation 1 originally asked for 'EXISTENCE, in interval "
        "arithmetic, of a solution of the self-similar profile system OF THE DISSIPATIVE "
        "EQUATION'. AT BCG's SCALING NO SUCH STATIONARY SYSTEM EXISTS. The self-similar "
        "reduction (eq:DS) that Thms 1.1/1.2 solve is the EULER system -- which is why the "
        "profiles are written (U^E, S^E) (BCG l.217, l.220), and why Thm 1.3 states its "
        "Navier-Stokes conclusion as ASYMPTOTICALLY self-similar blow-up. Dissipation "
        "never enters a stationary profile equation; it enters the dynamically rescaled "
        "system as the non-autonomous forcing F_dis, whose e^{(2-r+(1-r)/alpha)s} "
        "prefactor is s-independent at exactly one exponent, the borderline "
        "r = 2*gamma/(gamma+1) where delta_dis = 0. Taken literally, the original wording "
        "would have sent Phase 1 after an object that does not exist. "
        "VERIFIED INDEPENDENTLY BY LEG 266, NOT INHERITED: BCG arXiv:2208.09445 "
        "re-downloaded from arxiv.org/e-print and read at full LaTeX source, 6898 lines, "
        "e-print tarball md5 45ea63c45a1a199ecfb4dc4a15431600 -- matching the verifier's "
        "pin, obtained by a separate download. The window arithmetic (7/6, 1.1909830) is "
        "evaluated from BCG's own (eq:r:restriction) and (eq:rstar) at gamma = 7/5. "
        "SCOPE: this changes the wording of obligation 1 and its one-line restatement in "
        "the journal's sec 8, and nothing else. It does NOT change the gate answer, the "
        "named candidate, the compressible-vs-Clay flag, the Wall-2 statement, the "
        "Leray-projector finding, the CONDITIONAL tier, or the Clay odds -- verify_251 "
        "confirmed all of those at source and they stand byte-for-byte. The leg's own "
        "architecture description ('the computer-assisted part encloses the INVISCID ODE "
        "profile ... the viscous term is then handled analytically, by a parameter "
        "restriction, in the stability step') was ALREADY CORRECT; this was an internal "
        "inconsistency in one obligation's wording, not a misreading of the papers. "
        "Obligation 4 was already pointing at the correct locus and is unchanged."
    ),
    "ANSATZ_CORRECTION_2026_08_07": (
        "THE ANSATZ RE-NAMED TO n = 3 by leg 275 (Route-P0TC2). SOURCES OF THE CORRECTION, "
        "BOTH CITED: leg 265 (experiments/journal/leg_265.md on branch leg/265-p2c-v1, its "
        "section 'This corrects leg 251's ansatz, and the correction is load-bearing') AND "
        "writeup/novelty/verify_265.md sec B, the independent verification of leg 265, "
        "which re-derived the finding from BCG's own equations rather than inheriting leg "
        "265's framing. "
        "WHAT WAS WRONG: this leg's ansatz named r^(n) with 'n odd and LARGE', citing BCG "
        "Thms 1.2/1.3. BCG's Lemma k (l.579-586) makes k a MONOTONE BIJECTION "
        "[1, r*) -> [1, infinity), with r_j defined by k(r_j) = j. Large n therefore forces "
        "r_n -> r*, i.e. r^(n) deep INSIDE the dominance window (7/6, r*) -- the region "
        "where BCG's theorem already works, and precisely the region THIS LEG'S OWN "
        "OBLIGATION 4 FORBIDS ('show it is NOT contained in BCG's dominance regime', "
        "glossed under obligation 1 as r <= 7/6). The named ansatz and obligation 4 pulled "
        "in OPPOSITE directions. A second route reaches the same contradiction: Thm 1.3, "
        "cited here for the Navier-Stokes conclusion, requires delta_dis > 0, hence "
        "r > 7/6, hence inside the window again. "
        "THE MAGNITUDE THAT SETTLES IT: leg 265 evaluated k at the borderline in closed "
        "form from BCG's (eq:k_asquotientDZ1) l.600, (eq:R1) l.526 and (eq:def_R2) l.547 -- "
        "k(7/6) = 16.3479210517 -- so the smallest ODD branch BCG's own NS theorem can use "
        "is n >= 17, and the odd branches lying ENTIRELY inside the target window (1, 7/6] "
        "are {3, 5, 7, 9, 11, 13, 15}, seven of them. verify_265 re-derived "
        "k(7/6) = 16.34792105166133848739901684613 from hand-transcribed equations in "
        "50-digit arithmetic, AGREEING TO 15 SIGNIFICANT FIGURES, and in doing so resolved "
        "a typographic ambiguity in (eq:def_R2) that would otherwise have displaced the "
        "branch placement by ~4.6x; it also checked the seven-branch count against an n = 1 "
        "off-by-one using BCG's own restriction to 'odd n >= 3' (l.372). "
        "WHY n = 3 AND NOT THE ARITHMETICALLY CHEAPER n = 15: n = 3's existence is "
        "UNCONDITIONAL -- BCG Thm 1.1 (l.214) reads 'Let gamma in (1, +infinity). There "
        "exists r^(3)(gamma) in (r_3(gamma), r_4(gamma))', with no 'large enough' and no "
        "gamma-restriction, and 7/5 is in (1, infinity). n = 15 carries |delta_dis| <= "
        "0.0141, 31-41x smaller than n = 3's 0.4302-0.5778, BUT it depends on BCG Thm 1.2's "
        "'odd and large enough', which BCG never quantify; whether 15 qualifies is OPEN and "
        "is reported open, not assumed either way. This leg takes the unconditional branch. "
        "PRICE, STATED NOT HIDDEN: on r^(3) the dissipative forcing F_dis GROWS like "
        "e^{+0.5 s}, against an architecture BCG calibrated for decay at rate <= 0.1459 "
        "(leg 240) -- the same cost obligation 1 already accepted when it asked for the "
        "stability step WITH F_dis RETAINED at r <= 7/6. This correction makes the ansatz "
        "consistent with that obligation; it does not make the obligation easier. "
        "SCOPE: it changes the ANSATZ (and the object line's r^(n) -> r^(3)), its one-line "
        "restatement in the journal's sec 8, and the theorem citation attached to the "
        "'existence is already published' point -- and nothing else. It does NOT change the "
        "gate answer (YES, 6 of 14 survive, 4 unconditionally), the object (3D isentropic "
        "compressible Navier-Stokes at gamma = 7/5), the compressible-vs-Clay flag, the "
        "Wall-2 statement, the Leray-projector finding, the CONDITIONAL tier, the five "
        "obligations (1-5 BYTE-IDENTICAL, including leg 266's re-posed obligation 1), or "
        "the Clay odds (~0.05%, unmoved). "
        "CONSISTENCY READ, SAME PASS (leg 275's gate clause d): every r/window statement in "
        "this leg's three files was re-read -- obligation 1's (7/6, r*) arithmetic and its "
        "r <= 7/6 gloss, obligation 4, the journal's sec 8 restatement, and this runner's "
        "and the JSON's mirrors of each. ALL ALREADY POINT AT r <= 7/6, the correct side; "
        "the defect was confined to the ansatz naming plus the two citation surfaces that "
        "inherited it. One statement was checked specifically and CLEARED: the 'first 10000 "
        "(W_j, Z_j) Taylor coefficient pairs at r = r*' line is BCG's OWN VERBATIM sec 1.3 "
        "wording about THEIR computer-assisted step (transcribed at "
        "experiments/journal/leg_174.md:129-132), not a claim about this certificate's "
        "r-coverage, so it is correct as written and left untouched. NO THIRD INSTANCE OF "
    ...
    doc = {
        ...
        "negative_control": {
            ...
            "why_this_leg_is_not_the_same_mistake": [
                ...
                "IS ALREADY A PUBLISHED THEOREM (BCG Thm 1.1 at the named n = 3, "
                "unconditional for every gamma > 1; Thms 1.2/1.3 cover large odd n), so "
                "the certificate "
