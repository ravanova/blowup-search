...
## Arc 7 — independent confirmation (legs 436–440, **OPEN**)

Arc 7 does not re-derive the 2026 manuscript and does not referee it. It runs the Lean
kernel to completion on the two exported theorems, and it writes down what that does
and does not establish. **No unit in this arc is above Tier 2, and no unit moves an
`L1 → L4` link.** The arc's own quartet — `writeup/7_confirmation/`, BLOG + TECHNICAL +
evidence script + figure — is `K4`'s deliverable and **is not written yet**, which is
why the `B/T`, `E` and `F` columns below are empty rather than pending.

| Route | Gate / headline | R | D | B/T | E | F | Docs |
|---|---|---|---|---|---|---|---|
| `K0` — the charter (leg 436) | The user's rulings of 2026-09-10 recorded; arc 6's kernel check re-described as what it was — `#print axioms` on a build that never completed | — | — | — | — | — | `CORRECTIONS.md` §72 · [journal](../experiments/journal/leg_436.md) |
| `K1` — the kernel check (leg 437) | **GREEN in three environments** on pin `8937a8f4`: `lake build` completes (11251 jobs, 0 errors); `#print axioms` = `[propext, Classical.choice, Quot.sound]` for `navier_stokes_breakdown_R3` and `_periodic`; `sorryAx` **not reachable**. Three limits banked unsoftened — mathlib **replayed from cache, not rebuilt**; the statements are Fefferman **(C)/(D), not Theorem 1.1**; a second machine is not a second agent, so the unit is **`UNVERIFIED`** until `K2` slot 5 reproduces it blind | Y | Y | — | — | — | [prereg](../experiments/journal/leg_437_prereg.md) · [amend](../experiments/journal/leg_437_prereg_amend.md) · [journal](../experiments/journal/leg_437.md) · `CORRECTIONS.md` §73 |
| `K2` — the Lean's gaps (leg 438) | **IN FLIGHT** (run 2, prereg pushed before dispatch). Five `PARTIAL` statements, the four `sorry`, the comparator's four `NOT-ESTABLISHED` checks, DeepMind byte-identity re-verified at source, and a blind adversarial verifier in slot 5. Run 1's four worker files reached `main` with no prereg and no branch — the §3g step-1 defect, recorded at `CORRECTIONS.md` §74; **they are banked worker files, not findings** | — | partial | — | — | — | [prereg](../experiments/journal/leg_438_prereg.md) · `CORRECTIONS.md` §74 |
| `K3` — wave 4 redux (leg 439) | **NOT STARTED**; prereg pushed and once amended before any run. Wave 4's numerical gates rebuilt on one rule — *evidence is a two-route agreement on a quantity the adversary cannot choose* — and a gate that cannot be put in two-route form is **dropped, not weakened** (Q6 was dropped on exactly that test, §75). The arc's composition floor and its `W4` attack | — | — | — | — | — | [prereg](../experiments/journal/leg_439_prereg.md) · [amend](../experiments/journal/leg_439_prereg_amend.md) · `CORRECTIONS.md` §75 |
| `K4` — the verdict (leg 440) | **NOT STARTED.** `writeup/7_confirmation/`: the full quartet plus one document written for outsiders, leading with the verdict and never with the fragments; states what a green kernel does and does not establish; takes **no position** on priority | — | — | — | — | — | — |
