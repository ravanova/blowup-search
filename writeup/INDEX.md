...
| Route-FSB v1 (leg 301) — **PARKED, ESCALATED** | The fourth space/basis screen the re-posed 2026-08-06 ban's lift condition asks for: 14 candidates enumerated and screened on paper against the three MEASURED death mechanisms — gate answered **YES**, one survivor (Malmquist–Takenaka/Christov rational Hardy basis), and a yes here is **escalation**: the scoping-leg spec is written, the ban is **NOT** lifted and nothing is built | Y | Y | Y | Y | fig67 | [T](4_p2_lottery/TECHNICAL_P2_ROUTEFSB_V1.md) · [B](4_p2_lottery/BLOG_P2_ROUTEFSB_V1.md) — `fig64`–`fig66` deliberately skipped, left to legs running concurrently with this one; `fig67` was declared in `experiments/journal/leg_301.md` before any artifact existed |
