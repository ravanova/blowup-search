...
CAPABILITIES = [
    ...
    {"module": "solver/arc6_residual_ledger.py",
     "object": "exact-rational scaling ledger for a concentrating axisymmetric "
               "blowup momentum residual, with a route-4 contrast column "
               "(leg 419, arc 6 unit U3)",
     "holds": ("Scale(a,b) = the exponent of q in q^(a+b*h), carried as a pair "
               "of Fractions so every balance is EXACT and there is no "
               "tolerance anywhere to widen; GeometryLedger (ell_r, ell_z, "
               "core_volume, u_theta/u_z/u_r, Re_theta/Re_r, the three leading "
               "rates, the q^(2h) expansion parameter, leading_tangential_"
               "residual); PulseLedger (A_wave SOLVED from the cancellation "
               "requirement rather than transcribed, ell_wave, "
               "stress_divergence, stress_cancels_residual); EnergyLedger "
               "(E_core, D_core, critical_h, admissible_at); CorrectionCycle "
               "(sigma_j = 1/5 + j/10, stages_to_reach); manuscript_ledger() "
               "re-derives all 23 quantities and compares each against "
               "MANUSCRIPT_PRINTED; route4_contrast() puts THIS repository's "
               "pinned alpha=1 object in the same units"),
     "validated": ("all 23 re-derived scales equal the values OpenAI's "
                   "manuscript prints (sha256 0e779481c4da40bd..., pinned leg "
                   "417) -- exact equality of Fractions, not agreement to a "
                   "tolerance; the three leading rates coincide at exactly "
                   "q^-1; the pulse Reynolds-stress divergence equals the "
                   "leading tangential residual at exactly q^(-3/2-h); the "
                   "energy and dissipation conditions have the SAME threshold "
                   "h=1/6, checked rather than asserted; route4_contrast "
                   "REPRODUCES two banked results from exponent arithmetic "
                   "alone -- the L^2 deficit is exactly 1/2, the same 1/2 as "
                   "leg 381's cutoff bill, and the L^3 shell integrand at "
                   "alpha=1 is exactly r^-1, i.e. the log-divergence PB2 (leg "
                   "410) measured numerically at 139.287 per decade. SEVEN "
                   "PLANTED CONTROLS, each with a twin, fire in BOTH "
                   "directions: C1 amplitude q^(-1/2-h) fails the cancellation "
                   "(q^(-3/2-2h) vs q^(-3/2-h)); C2 an isotropic core breaks "
                   "the three-rate coincidence AND kills the expansion "
                   "parameter; C3 h=1/6 exactly FAILS, C4 h=1/5 fails and "
                   "h=1/7 passes; C5 a zero-step correction cycle does not "
                   "reach flatness; C6 alpha=3/2 is itself CRITICAL for L^2 "
                   "(this control was WRITTEN WRONG and the ledger caught it, "
                   "recorded in the test file rather than quietly repaired -- "
                   "it is why leg 381's bill reads alpha>1.5 and not >=1.5), "
                   "while alpha=8/5 and alpha=2 pass; C7 corrupting one printed "
                   "value makes the comparison fail and restoring it makes it "
                   "pass. CEILING: exponent bookkeeping. A consistent ledger is "
                   "a NECESSARY condition on a construction and nowhere near a "
                   "sufficient one -- it cannot see whether the objects it "
                   "counts exist. Nothing is integrated anywhere in the module, "
                   "deliberately: CORRECTIONS.md 52-54 record two integrals "
                   "here that looked convergent on a truncation and were not"),
     "test": "test_arc6_residual_ledger.py"},
    ...
    {"module": "solver/dssp_decay_enclosure.py",
     ...
     "holds": ("an OUTER enclosure of P_cert = { p >= 0 : exists C > 0 with "
               ...
               "consistency within a stated relative accuracy the CALLER owes. The "
               "rel_tolerance mode is PRE-REGISTERED (leg 386, Route-DTOL): width = "
               "4*log(1+delta)/log(R1/R0) in closed form via predicted_width(); delta "
               "AND the hypothesis in force are recorded on EVERY return path including "
               "all three INCAPACITY paths, as rel_tolerance / tolerance_mode ('exact' "
               "or 'relative') / predicted_width_exact_power_law / hypothesis / "
               "hypothesis_detail / conditional_on, default delta 0.0 unchanged. An "
               "omitted hypothesis is recorded UNDECLARED, never absent and never "
               "silently forgiven (standing rule, DM cycle 11g: a "
               "certificate-without-hypothesis is no certificate); cells mode DISCHARGES "
               "the enclosure hypothesis and records EXACT-INTERVAL-EVALUATION, nodes "
               "mode records NODES-ONLY so the weaker mode declares its weakness in the "
               "row; MONOTONE/MODULUS/MONOTONE+MODULUS pass through byte-identically "
               "from solver/dssp_decay_samples.py (leg 385). Also holds "
               "cutoff_admissible_delta_window(), which measures the CLAY_OBLIGATIONS §4 "
               "composed set D = { delta >= 0 : INTERVAL and p_lo > alpha_threshold } "
               "directly rather than by composing two legs' laws"),
     "validated": ("test_dssp_decay_enclosure.py, 18/18. Exact power laws p0 = 0.5, 1, "
                   ...
                   "data). Leg 386 re-measured the tolerance numbers FRESH under "
                   "pre-registration, leg 382's having been post-hoc: the width law "
                   "measures/closed-form 1.005051-1.005074 over nine decades of delta, "
                   "coefficient 0.87284 on [10,1000] vs 1.74567 on [10,100] (ratio "
                   "1.9999989, predicted exactly 2) -- so 0.8686 = 4/log 100 is a WINDOW "
                   "property, not an instrument constant. delta* = 0.31569700 "
                   "(two-power) / 3.35286818 (cutoff) / 0.06973929 (log) / 0 (exact), "
                   "reproducing leg 382's leads to 5e-9 / 5e-8 / 4e-6 and lying "
                   "0.98%/2.94%/9.46% BELOW the closed form exp(E_inf(log phi))-1, which "
                   "is an UPPER BOUND on the measured delta* and not an equality. At "
                   "delta* all three mismatches certify INTERVAL with truth inside the "
                   "exact local-slope range and widths <= 1.3e-15; at 0.9*delta* and "
                   "0.5*delta* all six remain EMPTY, so the mode is not a fitting knob. "
                   "MEASURED LIMIT, not hidden: the tolerance buys ZERO headroom on the "
                   "exponent threshold -- over 30 configurations the composed window is "
                   "non-empty IFF the realised certified centre already exceeds the "
                   "threshold (0 exceptions, crossover located at the centre to 1e-6), "
                   "so at the banked Type-I alpha = 1 the §4 window is EMPTY. Leg 381's "
                   "delta < (alpha_c-1)/0.434 is conservative by 1.33x-4.06x and never "
                   "optimistic; its implied demand alpha_c > 1+0.434*delta* is REFUTED "
                   "(the half-width vanishes at delta*, not at 0). "
    ...
    {"module": "solver/hookstep_newton.py",
     "object": "Newton-GMRES-hookstep trust-region globalisation "
               "(Viswanath 2007), matrix-free, plus its RPO driver "
               "(PROG-R4 unit U1 / milestone M1)",
     "holds": ("arnoldi (modified Gram-Schmidt with one reorthogonalisation "
               "pass, RAW Hessenberg returned -- no Givens -- because the "
               "hookstep needs an SVD at many radii); hookstep_subproblem "
               "(min ||H y - beta e1|| subject to ||y|| <= delta, solved by "
               "SVD with y(mu) = V diag(s/(s^2+mu)) g and mu found by "
               "bisection, returning a CANCELLATION-FREE predicted reduction "
               "2 y.(H^T rhs) - ||H y||^2 in squared currency); "
               "newton_hookstep (one Arnoldi build per Newton iteration with "
               "an optional GMRES relative-residual early exit (rtol), which "
               "is what makes a large nominal max_gmres affordable when one "
               "matvec is a full time integration; "
               "newton_hookstep (one Arnoldi build per Newton iteration with "
               "the whole radius loop inside it, ratio test rho = "
               "actual/predicted, per-iteration and per-radius-trial ledger, "
               "Jacobian-action and residual-evaluation counters). The RPO "
               "driver newton_hookstep_rpo lives in "
               "solver/kolmogorov2d_nkbasin.py and reuses that module's "
               "extended_residual unchanged: one 'epoch' per fixed phase-"
               "condition reference (the moving Poincare section), with the "
               "trust-region radius THREADED across epochs, T > 0 enforced "
               "as a domain constraint the radius loop rejects into, and a "
               "domain-safe finite-difference action that shrinks eps rather "
               "than probing through T = 0"),
     "validated": ("32/32 checks in test_hookstep_newton.py. The GMRES early "
                   "exit is measured to stop the build at k=8 instead of 60 "
                   "on a clustered spectrum, using 8 matvecs instead of 60, "
                   "with the relative GMRES residual at the stop verified "
                   "below the requested rtol (2.36e-7 for rtol=1e-6) so it is "
                   "a stopping test and not a truncation; rtol=None "
                   "reproduces the unrestricted build exactly. MEASURED "
                   "LIMIT: on a DENSE RANDOM matrix of the same size and "
                   "conditioning the build runs the full 60 either way, "
                   "because GMRES stagnates there -- the saving is real but "
                   "it is a property of the spectrum, not a guarantee. "
                   "Arnoldi "
                   "relation ||J Q_k - Q_{k+1} H||/||.|| = 1.6e-16, basis "
                   "orthonormality 7.0e-16, happy breakdown detected at the "
                   "right k. Unconstrained subproblem matches np.linalg."
                   "lstsq to 7.0e-16; constrained solve lands ON the "
                   "boundary and reports mu > 0; ||y(delta)|| non-decreasing "
                   "and model residual non-increasing in delta. THE THREE "
                   "CHECKS THAT SEPARATE A HOOKSTEP FROM A LINE SEARCH: "
                   "(i) as delta -> 0 on the FULL Krylov space the step "
                   "rotates onto -J^T F, cos = 1.00000000 (on a TRUNCATED "
                   "space the limit is that gradient PROJECTED onto span(Q_k), "
                   "which is why the check is run at full dimension); "
                   "(ii) the constrained step is NOT parallel to the GMRES "
                   "step, cos = 0.8896, where a line search gives 1 exactly; "
                   "(iii) on F(x) = A(x + 10 x^3) with cond(A) = 1e3, dim 6, "
                   "Krylov 5 -- a construction where J = A diag(1+30x^2) is "
                   "nonsingular EVERYWHERE, so ||F|| provably has no spurious "
                   "local minimum and the unique root is x = 0 -- leg 353's "
                   "own 8-halving line search on the SAME direction returns "
                   "line_search_failed at ||F||=5.030 (iteration 13) and "
                   "STILL stalls at ||F||=3.967 with 200 halvings, while the "
                   "hookstep converges to ||F||=1.95e-10 in 51 iterations "
                   "(||x||=1.7e-7), needing up to 6 radius trials: the "
                   "deficit is the DIRECTION, not the halving budget. "
                   "Re-solving at 50 further radii costs 0 extra Jacobian "
                   "actions (8 built the basis). MILESTONE M1 (measured, "
                   "N=24, Re=60, n=4, dt=0.01, tol=1e-8, max_newton=8, "
                   "max_gmres=15): through the new layer the laminar-fixed-"
                   "point control reduces ||R|| 19.110 -> 2.753e-2, i.e. "
                   "99.8560% monotone, AT OR ABOVE leg 353's recorded "
                   "99.3426310647174%, whose line-search column reproduces "
                   "bit-for-bit on the same inputs (99.3426%). MEASURED "
                   "LIMIT, recorded not tuned away: this control is rank-"
                   "deficient by 2 -- at an x-independent fixed point "
                   "||dR/ds|| = 0 EXACTLY and ||dR/dT|| = 0.0212 against "
                   "9.775 for a unit state direction -- and ||R|| at the "
                   "exact laminar point is 0.0794, a time-discretisation "
                   "floor, so NEITHER globalisation can reach 1e-8 here and "
                   "the milestone is stated as a reduction, not a "
                   "convergence. CEILING: M1 is a BUILD unit. It shows the "
                   "layer did not break a solver on a problem whose answer "
                   "is known analytically; it makes NO claim about whether "
                   "the hookstep recovers a published RPO -- that is gate G1 "
                   "(unit U3), pre-registered in experiments/journal/"
                   "prog_r4_prereg.md and NOT answered here. CLAY: no L1-L4 "
                   "link moved; CLAY_OBLIGATIONS §4 and both §6 obligations "
                   "remain OPEN"),
     "test": "test_hookstep_newton.py"},
]
